function doGet(){
  var template = HtmlService.createTemplateFromFile('libro_mayor');
  template.pubUrl = "https://script.google.com/macros/s/AKfycbwImyb1nX1M6atwIQZsk2yoIQVxPyoCdBcQFuspR1BOStAMylc91uv5ThWoCFWzJ9iI/exec";
...
for (var i = 1; i < cobranzasData.length; i++) {
  ...
  for (var j = 1; j < deudoresData.length; j++) {
    ...
    if (patenteCobranzas === patenteDeudores) {
      ...
      var importe = parseInt(cobranzasData[i][11].replace("$", "").replace(",", "")); // CorrecciÛn aquÌ
