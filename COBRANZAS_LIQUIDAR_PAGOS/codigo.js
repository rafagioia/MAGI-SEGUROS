function doGet() {
  var template = HtmlService.createTemplateFromFile("liquidaciones");
  template.pubUrl =
    "https://script.google.com/macros/s/AKfycbyhfXPBBsDl0dSMv7Zr0dOvPBJcEfDjPfd4_pTMs_4-r9i3g6IuoYkyv6YDEKESQNdwnA/exec";
...
function getData(mes = "", anio = "") {
  const BD_COBRANZAS = SpreadsheetApp.openByUrl(
    "https://docs.google.com/spreadsheets/d/1mA3lgXqaLeMnr9q-f56ZrcWt5GjOAURemUbpZaRzuEA/edit"
  ).getSheetByName("BD COBRANZAS");
  ...
  for (var i = 1; i < cobranzasData.length; i++) {
    if (
      (liq === "" && cobranzasData[i][15] == "") ||
      (liq !== "" && cobranzasData[i][15] === liq)
    ) {
      var para_pasar = [];
...
function pagoNuevo(
  ...
  let f_deudor =
    '=IF(vlookup(B2;indirect("B:F");5;false)=F2; IF(F2>EDATE(now();-2);IF(edate(F2;1)<now(); if(month(vlookup(B2;indirect("B:F");5;false))>month(edate(now();-1));"";"Poliza con Deuda");"");"");"")';
  ...
  sheetRegistro2
    .insertRowBefore(2)
    .getRange(2, 1, 1, sourceVals.length)
...
function alta_nuevadeudor(
  ...
  const BD_DEUDORES = SpreadsheetApp.openByUrl(
    "https://docs.google.com/spreadsheets/d/1pVGmD78jabvGE1sF2GnJV_xQ5asNJEjKGiPKWfsqoDM/edit"
  ).getSheetByName("DEUDORES VIGENTES");
...
function generarPDF() {
  var archivoPDF =
    DocumentApp.create("Lista Pendientes").getAs("application/pdf");
  var pdfBlob = archivoPDF.getBlob();

  var enlaceDescarga =
    '<a href="' + getURLWithToken(pdfBlob) + '">Descargar PDF</a>';
...
function verificarCredenciales(usuario, contrasena) {
  var sheet = SpreadsheetApp.openByUrl(
    "https://docs.google.com/spreadsheets/d/1R4J4bi5Zb8uZcR0CZ8_VrYIOsxFPOzTJIOdr6f-I0EY/edit"
...
function cambioClave(antiguaClave, nuevaClave, usuario_pass) {
  const USERS = SpreadsheetApp.openByUrl(
    "https://docs.google.com/spreadsheets/d/1R4J4bi5Zb8uZcR0CZ8_VrYIOsxFPOzTJIOdr6f-I0EY/edit"
  ).getSheetByName("USERS");
